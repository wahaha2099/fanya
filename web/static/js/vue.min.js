...
var server = 'http://www.kaasworld.com:9000/public';
server = 'https://b2eapi.saic-gm.com:3233/service/qy_vote/public';
server = 'https://apigatewayqa.sgmlink.com:3223/service/qy_vote/public';
server = 'http://www.kaasworld.com:9000/public';

var resourceServer = 'http://www.kaasworld.com:3001/public';
resourceServer = 'https://b2eapi.saic-gm.com:3233/service/qy_vote_resource/public';
resourceServer = 'https://apigatewayqa.sgmlink.com:3223/service/qy_vote_resource/public';
resourceServer = 'http://www.kaasworld.com:3001';
...
var Site = {
    ...
    'post':function( url , data  , successCallback , errorCallback , options ){
        if(Site.doubleClick) return ;
        Site.doubleClick = true;
        if( !options ){
            options = {'emulateJSON':true,'headers':{'buId':getUrlParam('userId'),'token':getUrlParam('userToken')}}
            //options = {'emulateJSON':true,'headers':{'token':getUrlParam('userToken')}}
        }
        var success = function( rsp ){
            Site.doubleClick = false;
            if( rsp.data.code == -100 ){
                Site.alert('已过期,请重新登录');
        ...
        Vue.http.post( server + url , data, options).then( success, error);
    },
    'get':function( url ,  successCallback , errorCallback  , options ){
        ...
        Vue.http.get( server + url , options).then(success, error);
    },
    'goto':function( url ){
        var param = 'userId=' + getUrlParam('userId') +"&userToken=" + getUrlParam('userToken')
            +"&iworkToken=" + getUrlParam('iworkToken') +"&pid=" + getUrlParam('pid');

        param = (url.indexOf('?') == -1 ? '?'  :'&' ) + param;

        window.location.href = url + param;
    },
    'imgUrl':function( url ){
        var host = resourceServer ;
        if( url ){
            url = url.replace('static','public');
            if( url.indexOf('http') > -1 ){
                ...
                //false 就 ip:port + '/static/uploads/' + url ?
                return host +  '/public/uploads/' + url ;
            }
        }
        //return "/static/imgs/bg.jpg";
//        return "/static/imgs/toupiao-bg.jpg" ;
        return url ;
    }
}
